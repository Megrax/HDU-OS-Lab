...
int main()
{
    ...
    sem_t *read_mutex1;
    sem_t *read_mutex2;
    write_mutex = sem_open("write", O_CREAT, 0666, 0);
    read_mutex1 = sem_open("read1", O_CREAT, 0666, 0);
    read_mutex2 = sem_open("read2", O_CREAT, 0666, 0);

    if (pipe(fd) != 0) // *param* fd[0] to read fd[1] to write *return* 0 on success -1 on error
    {
        printf("创建管道失败\n");
        exit(1);
    }
    ...
    {
        printf("Child procees 1 starts writing...\n");
        close(fd[0]);
